...
public class ReservationController {
    ...
    @ResponseBody
    private ListEmp getEmp(@RequestBody String city, User user){
        List<Employee> empList=empService.gettAll();
        ListEmp list=new ListEmp();
        List<Employee> result=new ArrayList<Employee>();
        List<User> listUser=userService.gettAll();
        for(int i=0;i<listUser.size();i++){
            if ((listUser.get(i).getEmail().equals(user.getEmail()))&&(listUser.get(i).getPassword().equals(user.getPassword()))){

                for (int j=0;i<empList.size();i++){
                    if (empList.get(j).getCity().equals(city)){
                        empList.get(j).setImage(IPaddress.IPPORT.toString()+empList.get(j).getImage());
    ...
    @ResponseBody
    private String deleteRes(@RequestBody Long id, User user){
        List<User> list=userService.gettAll();
        for(int i=0;i<list.size();i++){
            if ((list.get(i).getEmail().equals(user.getEmail()))&&(list.get(i).getPassword().equals(user.getPassword()))){
                    ...
                    if (scheduleList.get(j).getId_emp() == res.getEmp().getId_emp()) {
                        ...
                        if (res.getTime().equals(timeSchedule)) {
                            if (scheduleList.get(j).getReserve() == 0) {
                                ...
                                //schedule.setId_emp(user.getId_user());

                                resService.save(reservation);
                                scheduledService.save(schedule);
                                return "done";
                            } else {
                                return "";
                            }
